...
NEURON {
	...
	RANGE w_pre, w_post, w_pre_init, w_post_init, w, pathway
	RANGE s_ampa, s_nmda
...
DERIVATIVE state {
	...
	if(w_pre > bound*w_pre_init) {
		w_pre = bound*w_pre_init
	}
	if(w_pre < 0.0) {
		w_pre = 0.0
	}
	w_post = w_post + LTD_post + LTP_post
	if(w_post > bound*w_post_init) {
		w_post = bound*w_post_init
	}
	if(w_post < 0.0) {
		w_post = 0.0
	}
